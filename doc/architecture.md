...
| コマンド | 引数 | 説明 |
| --- | --- | --- |
| **`status`** | (opt)`-s` | 全プロセスの PID、稼働状態、Ready <状態を表示。</br>(opt)`-s`を付けると起動しているサービスの統計情報を表示。 |
| **`addr`** | `<PLC名>` | 指定したPLCのアドレス領域を表示。 |
| **`info`** | `<PLC名>`<br>(opt)`--watch [n]` | 指定したPLCの全メモリ（X / Y / M / D）の状態を一覧表示。</br>`--watch` を付与すると1秒周期で自動更新（デフォルト5回、nで回数指定可）。 |
| **`log`** | なし | 対話型ログビューアを起動。 |
