...
class DeviceEditorView(ft.Container):
    ...
    def create_signal_card(self, name, data):
        ...
        def add_pattern_row(val=0, dur=1000):
            v_in = ft.TextField(value=str(val), width=100, dense=True, label="Value")
            d_in = ft.TextField(value=str(dur), width=100, dense=True, label="ms", suffix="ms")
            row = ft.Row([
                v_in, d_in,
                ft.IconButton(ft.Icons.DELETE_OUTLINE, icon_color="red400", on_click=lambda _: (pattern_col.controls.remove(row), self.update()))
            ])
        ...
        card = ft.Container(
            ...
            content=ft.Column([
                ...
                pattern_col,
                ft.TextButton("Add Step", icon=ft.Icons.ADD_CIRCLE_OUTLINE, on_click=lambda _: (add_pattern_row(), self.update()))
            ])
        )
        # 回収用にメタデータを埋め込む
    ...
    def save_settings(self, e):
        try:
            ...
            self.svc["plc"] = {"host": self.host_field.value, "port": int(self.port_field.value)}
