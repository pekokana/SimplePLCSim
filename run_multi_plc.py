...
def parse_ladder_line(line: str):
    ...
    if tokens[0].upper() == "TON":
        ...
        return {"logic": logic_expr, "coil": out}
...
# -----------------------------
class PLC:
    def __init__(self, name, plc_conf, ladder_conf):
        self.name = name
    ...
    def print_status(self):
        ...
        print(f"{now} | {self.name} | X={self.mem.X} | M={self.mem.M} | Y={self.mem.Y}")
    ...
    def exec_ctu(self, rung):
        ...
        if current and not self.prev_inputs[inp]:
            self.C[cname] += 1
            print(f"[{self.name} CTU] {cname} = {self.C[cname]}")
...
# 複数PLC起動関数
# -----------------------------
def start_plc(plc_name, plc_conf_file, ladder_conf_file):
    ...
    plc = PLC(plc_name, plc_conf, ladder_conf)

    # Modbus起動（PLCごとにport指定）
...
if __name__ == "__main__":
    # 引数例: run_multi_plc.py plc_A.yaml ladder_A.yaml plc_B.yaml ladder_B.yaml ...
    args = sys.argv[1:]
    if len(args) % 2 != 0:
        print("PLCごとにPLC設定ファイルとラダー設定ファイルを指定してください")
        sys.exit(1)

    for i in range(0, len(args), 2):
        plc_conf_file = args[i]
        ladder_conf_file = args[i+1]
        plc_name = f"PLC_{i//2 + 1}"
        threading.Thread(target=start_plc, args=(plc_name, plc_conf_file, ladder_conf_file), daemon=True).start()
