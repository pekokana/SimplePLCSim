...
本 PLC シミュレータでは、**実機 PLC に近いタイマ / カウンタ概念**を内部メモリとして保持します。

| 種類 | 名称   | 説明                  |
| -- | - | - |
| T  | タイマ  | 時間経過を管理する内部デバイス     |
| C  | カウンタ | 入力の立上り回数を管理する内部デバイス |

これらは **PLC 内部メモリとして管理され、Modbus から直接は参照しません**。
（SCADA とはイベントや出力ビットを通じて連携します）
